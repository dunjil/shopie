...
class ViewStockScreen extends StatefulWidget {
  ...
  ViewStockScreen({this.productId});
  @override
...
class _ViewStockScreenState extends State<ViewStockScreen> {
  ...
  _getProductDetails() async {
    ...
    await _http.post(kGetProductDetailsUrl, headers: {
      'Content-Type': 'application/x-www-form-urlencoded'
    }, body: {
      "business_id": businessId,
      "product_id": widget.productId,
    }).then((response) {
      debugPrint("the data is: ${response.body}", wrapWidth: 14024);
      Map listData = json.decode(response.body);
      product = Product.fromJson(listData);
      setState(() {
        _stockName.text = product.item;
        _code.text = product.code;
        _unitPrice.text = product.unitPrice.toString();
        _oldPrice.text = product.unitPrice.toString();
        ...
        _quantityLimit.text = product.quantityLimit.toString();
  ...
  _getProductImages() async {
    ...
    await _http.post(kGetProductImagesUrl, headers: {
      'Content-Type': 'application/x-www-form-urlencoded'
    }, body: {
      "product_id": widget.productId,
    }).then((response) {
      ///List data = jsonDecode(response.body);
      debugPrint("Response is" + response.body, wrapWidth: 14024);
      // if (data[0]['file'] != null) {
      //   setState(() {
      //     image1 = base64ToImage(data[0]['file']);
      //     print("We don pass Here");
      //     im1 = true;
      //   });
      // }
    });
  ...
  _getProductVideo() async {
    setState(() {
      _isLoadingVideo = true;
    });
    await _http.post(kGetProductVideoUrl, headers: {
      'Content-Type': 'application/x-www-form-urlencoded'
    }, body: {
      "product_id": widget.productId,
    }).then((response) {
      debugPrint("Video Response: " + response.body, wrapWidth: 14024);
    });
    setState(() {
                        ...
                        child: addImageButton(
                            ...
                            onPress: () {
                              im3
                                  ...
                                  : _imageErr(
                                      message: "You have to set Image 3 first");
                            },
                            imgBool: im4)),
                    Expanded(
                      child: addImageButton(
                ...
                _vid
                    ? FlatButton.icon(
                        ...
                        label: Text(
                          "Play Video",
                          style:
                              TextStyle(color: Colors.white, fontSize: tm * 2),
                        ),
                      )
                    : Text(""),
                _vid
  ...
  _updateProduct() async {
    ...
    try {
      _http.Response response = await _http.post(kUpdateProductUrl, headers: {
        'Content-Type': 'application/x-www-form-urlencoded'
      }, body: {
  ...
  updateProductFiles(String product_id) async {
